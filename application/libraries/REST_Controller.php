...
abstract class REST_Controller extends CI_Controller {
    ...
    /**
     * The arguments for the all request method
     *
     * @var array
     */
     ...
     * @author Sachin
     */
    private function api_log_entry() {

        $field = array();
        $field["method"] = isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '';
        $field["full_url"] = BASE_URL . (isset($_SERVER['REDIRECT_QUERY_STRING']) ? $_SERVER['REDIRECT_QUERY_STRING'] : '');
        $field["request_data"] = json_encode($this->request_data);
        $field["header_data"] = json_encode(apache_request_headers());
        $field["source"] = isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '';
        $field["created_on"] = get_date_time();
        $this->rest->db->insert(TABLE_API_LOG, $field);
    }
     ...
     *
     * @param  boolean  $is_check to check auth token or not.
     * @param  boolean  $message callback message.
     * @param  boolean  $db_user_id callback user id from db if success.
     * @return boolean
     */
    public function is_valid_auth_token($is_check = true, &$message, &$db_user_id) {
        // set default to true if value is blank
        if (!is_bool($is_check)) {
            $is_check = true;
        }
...
//        pr($all_headers);die;
        $final_auth_token = '';
        if ($is_check == true) {
            if (isset($all_headers['auth_token']) && trim($all_headers['auth_token']) != '') {
                $final_auth_token = $all_headers['auth_token'];
            } else {
        ...
        // return true if not to be checked
        if ($is_check != true) {
            $message = 'Success';
            ...
            $where_condition = array($final_auth_token);
            $sql = "select user_id from " . TABLE_ACCOUNTS_AUTHTOKEN . " where `key` = ? limit 1";
            $output = $this->rest->db->query($sql, $where_condition);
            $auth_row = $output->row_array();

            if (isset($auth_row['user_id']) && trim($auth_row['user_id']) != '') {
                $db_user_id = $auth_row['user_id'];
            }
            return true;
        } else {
            // cehck valid user and auth token
            if ($final_auth_token != '') {

                $user_id = (isset($all_headers['login_user_id']) && $all_headers['login_user_id'] > 0) ? $all_headers['login_user_id'] : 0;
                $auth_token = $final_auth_token;

                $where_condition = array($auth_token);
                ...
                $sql = "select * from " . TABLE_ACCOUNTS_AUTHTOKEN . " where `key` = ? {$sql_str} limit 1";
                $output = $this->rest->db->query($sql, $where_condition);
                $auth_row = $output->row_array();
                
                if (isset($auth_row['key']) && trim($auth_row['key']) != '') {
                    $message = 'Success';
                    $db_user_id = $auth_row['user_id'];
                    $this->request_data['auth_token'] = $auth_row['key'];
                    return true;
                } else {
                    $message = 'Wrong authentication credentials provided.';
                    return false;
                }
            } else {
                $message = 'Authentication credentials were not provided.';
